...
async def google_login() -> RedirectResponse:
    ...
    try:
        ...
        logger.exception(f"Failed to initiate Google OAuth login: {e}")
        return RedirectResponse(
            url=setting.frontend_url + "/login?error=oauth_init_failed",
            status_code=500,
        )
...
async def google_callback(
    ...
    try:
        ...
        if error:
            logger.error(f"Google OAuth callback returned error: {error}")
            return RedirectResponse(
        ...
        if prev_state != state:
            logger.error(f"OAuth state mismatch: expected={prev_state}, got={state}")
            return RedirectResponse(
